...
local displayInputs = require("displayinput")
...
function sync.syncallinput(client_socket, player)
  ...
  while 1 do
    ...
    if config.game == "superstarsaga" then
      current_input, their_input = modifyInputs(current_input, their_input, player)
    end

    displayInputs(current_input, their_input, player)

...
function sync.syncallinput(client_socket, player)
  ...
  local buttons = {"A", "B", "X", "Y", "L", "R", "Select", "Start", "Right", "Left", "Up", "Down"}
  local numbuttons = 1
  local unset = {}
  ...
  while 1 do
    ...
    if (emu.framecount() > config.latency) then
      while (peer_queue[emu.framecount()] == nil) do
        ...
        if(their_message == nil) then
          ...
          their_frame = tonumber(their_message:sub(numbuttons))
    ...
    current_input, their_input = modify_inputs(current_input, their_input, player)
    display_inputs(current_input, their_input, player)
